---
title: دليل التطوير
description: كيفية إعداد مشروع F5 XC API Specs وتشغيله واختباره والمساهمة فيه
i18n:
...
يقوم `make dev-install` بإنشاء بيئة افتراضية في `.venv/` وتثبيت المشروع مع الإضافات التطويرية (pytest, ruff, mypy).

### متغيرات البيئة

يتطلب التحقق المباشر من واجهة البرمجة:
...
بدون هذه المتغيرات، يعود `make validate` إلى وضع التشغيل الجاف (بدون استدعاءات فعلية).
...
تعمل خطافات ما قبل الإيداع (pre-commit hooks) تلقائياً عند الإيداع إذا تم تثبيتها:
...
لإضافة إصلاح تلقائي جديد لقاعدة Spectral إلى المُوفِّق (reconciler):
...
1. أنشئ الدالة في `spectral/functions/your-rule.js`. قم بتصدير دالة افتراضية تستقبل `(targetVal, options, context)` وتُعيد مصفوفة من كائنات `{message, path}`.
...
:::caution
تعمل الدوال المخصصة فقط في `spectral-pipeline.yaml`. لا تُضف `functionsDir` أو `functions` إلى `.spectral.yaml` -- يجب أن يبقى هذا التكوين متوافقاً مع Super-Linter والأدوات الخارجية الأخرى.
:::
...
- **جدول زمني**: يومياً الساعة 6 صباحاً بالتوقيت العالمي
- **الدفع إلى main**: عند تغيير `scripts/` أو `config/` أو `pyproject.toml`
- **التشغيل اليدوي**: مع خيارات للإصدار القسري أو التشغيل الجاف

يحتوي سير العمل على أربع مهام:

1. **validate** -- تنزيل المواصفات، تشغيل التحقق، فحص Spectral، المُوافقة، بوابة Spectral
2. **check-release-needed** -- مقارنة تجزئات المحتوى لتحديد ما إذا كان الإصدار مطلوباً
3. **release** -- تجميع وإنشاء إصدار GitHub (فقط إذا تغيّر المحتوى)
4. **update-docs** -- إعادة توليد `01-validation-report.mdx` وفتح طلب سحب إذا تغيّر

تقوم مهمة **الإشعارات** بإنشاء مشكلة (issue) على GitHub إذا فشلت أي مهمة.

## سير عمل الفروع وطلبات السحب

تتبع جميع التغييرات: **مشكلة -> فرع -> طلب سحب -> نجاح التكامل المستمر -> دمج**.

- أسماء الفروع: `feature/<issue>-desc`، `fix/<issue>-desc`، `docs/<issue>-desc`
- يجب أن تربط طلبات السحب بمشكلة (`Closes #N` في الوصف)
- فحوصات التكامل المستمر المطلوبة: `Check linked issues` و `Lint Code Base`
- يُفضَّل الدمج بالضغط (squash merge)؛ تُحذف الفروع تلقائياً بعد الدمج

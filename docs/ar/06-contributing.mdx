...
يتطلب التحقق المباشر من واجهة برمجة التطبيقات:
...
بدون هذه المتغيرات، يعود `make validate` إلى وضع التشغيل الجاف.
...
تعمل خطافات ما قبل الإيداع (pre-commit hooks) تلقائياً عند الإيداع إذا كانت مثبّتة:
...
لإضافة إصلاح تلقائي جديد لـ Spectral إلى المُوفِّق (reconciler):
...
يعمل سير عمل GitHub Actions (`validate-and-release.yml`) عند:

- **الجدولة**: يومياً في الساعة 6 صباحاً بالتوقيت العالمي (UTC)
- **الدفع إلى main**: عند تغيير `scripts/` أو `config/` أو `pyproject.toml`
- **التشغيل اليدوي**: مع خيارات لفرض الإصدار أو التشغيل الجاف

يتضمن سير العمل أربع مهام:

1. **validate** -- تنزيل المواصفات، تشغيل التحقق، فحص Spectral، المُوفَقة، بوابة جودة Spectral
2. **check-release-needed** -- مقارنة تجزئات المحتوى لتحديد ما إذا كان الإصدار مبرراً
...
تقوم مهمة **notify** بإنشاء مشكلة (issue) على GitHub إذا فشلت أي مهمة.

## سير عمل الفروع وطلبات السحب

تتبع جميع التغييرات المسار: **مشكلة -> فرع -> طلب سحب -> نجاح التكامل المستمر -> دمج**.
...
- يُفضّل الدمج بالضغط (squash merge)؛ تُحذف الفروع تلقائياً بعد الدمج

...
make release         # Genera il pacchetto di rilascio
```
...
make docs            # Genera il sito di documentazione completo
...
make lint           # ruff check + controllo formattazione
...
scripts/
  ...
  spectral_lint.py      # Fase 3/5: Adattatore per il linting Spectral
  reconcile.py          # Fase 4: Applica correzioni alle specifiche
  release.py            # Fase 6: Genera pacchetto di rilascio
...
Per aggiungere una nuova correzione automatica Spectral al riconciliatore:

1. **Aggiungi la regola** a `.spectral.yaml` con la severità desiderata.

2. **Abilita la correzione automatica** in `config/validation.yaml` sotto `spectral.auto_fix`:
   ```yaml
   spectral:
     auto_fix:
       your-new-rule: true
...
3. **Aggiungi il metodo di correzione** a `SpecReconciler` in `scripts/reconcile.py`:
   ```python
...
5. **Aggiungi i test** che coprano il nuovo metodo di correzione.
...
1. **validate** -- Scarica le specifiche, esegue validazione, lint Spectral, riconciliazione, gate Spectral
2. **check-release-needed** -- Confronta gli hash dei contenuti per decidere se un rilascio è necessario
3. **release** -- Crea il pacchetto e una GitHub Release (solo se il contenuto è cambiato)
4. **update-docs** -- Rigenera `01-validation-report.mdx` e apre una PR se ci sono modifiche

Un job **notify** crea una issue GitHub se un qualsiasi job fallisce.

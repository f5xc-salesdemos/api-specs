---
title: 구성 참조
description: validation.yaml 및 endpoints.yaml 구성 파일 참조
i18n:
...
:::caution
`validation.yaml`의 `base_url`, `tenant`, `namespace` 값은 일반적인 플레이스홀더입니다. 파이프라인을 실행하기 전에 환경 변수를 통해 사용자 환경에 맞게 **반드시** 구성해야 합니다:
...
| 필드 | 설명 |
|-------|-------------|
| `url` | 공식 F5 XC OpenAPI 사양 번들(ZIP)의 URL. |
| `output_dir` | 추출된 사양이 저장되는 디렉토리. |
...
| `stateful_testing` | 작업 간 링크 기반 상태 유지 테스트 활성화. |
...
| 필드 | 설명 |
|-------|-------------|
| `priority` | 여러 데이터 소스가 불일치할 때의 우선순위. |
| `fix_strategies` | 각 불일치 유형을 수정 동작에 매핑합니다. [적용된 수정 사항](/03-fixes-applied/)을 참조하세요. |
...
| `ruleset` | 사용할 Spectral 구성 파일(파이프라인에서는 `spectral-pipeline.yaml`을 사용). |
...
| `contact` | `info-contact` 수정기가 `info.contact`에 주입하는 연락처 정보. |
| `servers` | `oas3-api-servers` 수정기가 주입하는 서버 배열. |
| `security_scheme` | 모든 사양에 주입되는 보안 스키마 정의. |
...
| `version_from` | 버전 소스. `git`은 사양 메타데이터 날짜 + 패치 번호에서 버전을 파생합니다. |

## `config/endpoints.yaml`

라이브 API 유효성 검사에 사용되는 기준 엔드포인트를 정의합니다. 각 항목은 논리적 리소스 이름을 해당 사양 파일, API 그룹 및 CRUD 경로에 매핑합니다.
...
1. `specs/original/`에서 사양 파일명을 찾습니다(형식: `docs-cloud-f5-com.NNNN.*.ves-swagger.json`)
2. 위의 구조에 따라 `endpoints:` 아래에 항목을 추가합니다
3. `domain_files:` 아래에 우선순위 번호와 함께 도메인 파일 매핑을 추가합니다
4. 원하는 위치의 `test_order:`에 엔드포인트 이름을 추가합니다

:::tip
프레임워크는 `endpoints.yaml`에 나열된 항목과 관계없이 조정 과정에서 모든 268개 사양을 검증합니다. 엔드포인트 구성은 Schemathesis를 통해 라이브 API 테스트를 수행할 리소스를 제어합니다.
:::

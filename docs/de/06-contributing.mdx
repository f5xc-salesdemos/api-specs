...
description: >-
  Wie Sie das F5 XC API Specs-Projekt einrichten, ausführen, testen und dazu
  beitragen können
i18n:
...
Ohne diese fällt `make validate` in den Trockenlauf-Modus zurück.
...
make spectral-lint   # Spectral-Erkennungsmodus für Original-Spezifikationen
make reconcile       # Korrekturen aus beiden Berichten anwenden
make spectral-gate   # Spectral-Qualitäts-Gate für abgeglichene Spezifikationen
...
scripts/
  ...
  release.py            # Stufe 6: Release verpacken
...
1. **Fügen Sie die Regel** zu `.spectral.yaml` mit dem gewünschten Schweregrad hinzu.
...
- **Push auf main**: Wenn sich `scripts/`, `config/` oder `pyproject.toml` ändert
...
3. **release** -- Verpackt und erstellt ein GitHub-Release (nur wenn sich Inhalte geändert haben)

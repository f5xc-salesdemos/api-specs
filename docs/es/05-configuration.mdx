...
| `timeout` | Tiempo de espera de solicitud HTTP en segundos. |
...
Las variables de entorno tienen precedencia sobre los valores predeterminados del YAML en tiempo de ejecución.
:::
...
Diez categorías que corresponden a los tipos de restricciones del esquema OpenAPI. Cada una puede habilitarse/deshabilitarse de forma independiente.
...
| Campo | Descripción |
|-------|-------------|
| `priority` | Orden de preferencia cuando múltiples fuentes de datos no coinciden. |
| `fix_strategies` | Asocia cada tipo de discrepancia con su acción de corrección. Consulte [Correcciones Aplicadas](/03-fixes-applied/). |
...
| `gate.max_errors` | Máximo de errores permitidos en la compuerta post-reconciliación. `null` desactiva la verificación. |
| `gate.max_warnings` | Máximo de advertencias permitidas. `null` desactiva la verificación. |
...
| `include_validation_report` | Incluir el reporte de validación en el paquete de publicación. |
| `version_from` | Fuente de la versión. `git` deriva la versión de la fecha de los metadatos de la especificación + número de parche. |

## `config/endpoints.yaml`

Define los endpoints de referencia utilizados para la validación en vivo de la API. Cada entrada asocia un nombre lógico de recurso con su archivo de especificación, grupo de API y rutas CRUD.

---
title: Reglas de Linting Spectral
description: >-
...
El proyecto utiliza [Spectral](https://stoplight.io/open-source/spectral) para el linting de OAS3 con una arquitectura de dos configuraciones que separa el CI/herramientas externas de la lógica personalizada del pipeline.

## Arquitectura de Dos Configuraciones

### `.spectral.yaml` -- Configuración Base

Utilizada por GitHub Super-Linter y cualquier herramienta de CI externa. Extiende `spectral:oas` (el conjunto de reglas OAS incorporado). No contiene funciones personalizadas -- esto la mantiene compatible con herramientas que ejecutan Spectral sin acceso al directorio `spectral/functions/`.
...
La regla incorporada `path-params` de Spectral marca incorrectamente las APIs de F5 XC que utilizan nombres de parámetros con puntos como `{metadata.namespace}` y `{metadata.name}`. La regla incorporada no analiza los puntos en los nombres de parámetros, por lo que los reporta como no declarados.
...
| `oas3-unused-component` | warn | `_remove_unused_component` | No debe haber esquemas de componentes sin referencia |
| `operation-operationId-unique` | error | `_deduplicate_operation_id` | Todos los operationIds deben ser únicos |
| `oas3-valid-schema-example` | error | `_fix_schema_example` | Los ejemplos/valores por defecto deben coincidir con su esquema |
| `no-script-tags-in-markdown` | warn | `_strip_script_tags` | No debe haber etiquetas `<script>` en las descripciones |
...
Escanea `specs/original/` y genera `reports/spectral_report.json`. El reconciliador consume este reporte junto con el reporte de validación.
...
Escanea `release/specs/` y genera `reports/spectral_gate_report.json`. Retorna el código de salida 1 si las violaciones exceden los umbrales configurados.

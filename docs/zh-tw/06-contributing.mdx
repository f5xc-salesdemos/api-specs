---
title: 開發指南
description: 如何設定、執行、測試及貢獻 F5 XC API Specs 專案
i18n:
  sourceHash: 1dc0c369e7a0
  translator: machine
---

## 設定

```bash
# 複製儲存庫並安裝開發依賴
git clone https://github.com/f5xc-salesdemos/api-specs.git
cd api-specs
make dev-install

# 安裝 Spectral CLI（lint 階段必須）
npm install -g @stoplight/spectral-cli
```

`make dev-install` 會在 `.venv/` 中建立虛擬環境，並安裝含開發擴充套件（pytest、ruff、mypy）的專案。

### 環境變數

即時 API 驗證需要：

```bash
export F5XC_API_URL="https://example-tenant.console.ves.volterra.io"
export F5XC_API_TOKEN="your-api-token"
```

若未設定這些變數，`make validate` 將回退至乾跑模式。

## 執行管線

### 完整管線

```bash
make all
# 執行：download → validate → spectral-lint → reconcile → spectral-gate → release
```

### 個別階段

```bash
make download        # 取得上游規格
make validate        # 執行即時 API 測試（需要 API 權杖）
make validate-dry    # 乾跑模式（不呼叫 API）
make spectral-lint   # 對原始規格執行 Spectral 探索模式
make reconcile       # 根據兩份報告套用修正
make spectral-gate   # 對已修正規格執行 Spectral 品質門檻
make release         # 建置發佈套件
```

### 文件

```bash
make docs-generate   # 從報告重新產生 docs/01-validation-report.mdx
make docs            # 建置完整文件網站
make docs-serve      # 本地開發伺服器，支援熱重載
```

## 執行測試

```bash
make test           # 含覆蓋率的 pytest
make lint           # ruff check + format check
make typecheck      # mypy 型別檢查
make ci-test        # 以上三者全部執行（用於 CI）
```

若已安裝，Pre-commit hooks 會在提交時自動執行：

```bash
make pre-commit-install   # 安裝 git hooks
make pre-commit           # 手動執行所有 hooks
```

## 專案結構

```
scripts/
  download.py           # 階段 1：從 F5 取得規格
  validate.py           # 階段 2：即時 API 測試
  spectral_lint.py      # 階段 3/5：Spectral lint 適配器
  reconcile.py          # 階段 4：對規格套用修正
  release.py            # 階段 6：打包發佈
  generate_docs.py      # 從報告產生 MDX
  utils/
    auth.py             # F5 XC API 認證
    constraint_validator.py  # 約束測試 + 差異類型
    report_generator.py      # 報告格式化
    schemathesis_runner.py   # Schemathesis 測試編排
    spec_loader.py           # 規格 I/O + JSON 格式化

config/
  validation.yaml       # 管線設定
  endpoints.yaml        # 即時測試的基準端點

spectral/
  functions/
    f5-path-params.js   # 自訂 Spectral 函式

docs/                   # Starlight MDX 文件
tests/                  # pytest 測試套件
release/specs/          # 輸出：已修正的規格檔案
reports/                # 輸出：驗證與 Spectral 報告
```

## 新增修正方法

若要在修正器中新增 Spectral 自動修正：

1. **新增規則**到 `.spectral.yaml`，設定所需的嚴重性等級。

2. **啟用自動修正**，在 `config/validation.yaml` 的 `spectral.auto_fix` 下設定：
   ```yaml
   spectral:
     auto_fix:
       your-new-rule: true
   ```

3. **新增修正方法**到 `scripts/reconcile.py` 中的 `SpecReconciler`：
   ```python
   def _fix_your_rule(self, spec: dict, discrepancy: Discrepancy) -> dict | None:
       """Fix description."""
       # Modify spec in place
       # Return spec if changed, None if no change needed
       return spec
   ```

4. **註冊方法**到 `_apply_spectral_fix` 分派器中：
   ```python
   spectral_fixers = {
       # ... existing fixers ...
       "spectral:your-new-rule": self._fix_your_rule,
   }
   ```

5. **新增測試**以涵蓋新的修正方法。

## 新增 Spectral 規則

若要新增含 JavaScript 函式的自訂 Spectral 規則：

1. 在 `spectral/functions/your-rule.js` 中建立函式。匯出一個預設函式，接收 `(targetVal, options, context)` 並回傳 `{message, path}` 物件的陣列。

2. 在 `spectral-pipeline.yaml` 中註冊：
   ```yaml
   functions:
     - f5-path-params
     - your-rule

   rules:
     your-rule:
       description: "What it checks"
       message: "{{error}}"
       severity: error
       given: "$.paths[*]"
       then:
         function: your-rule
   ```

:::caution
自訂函式僅在 `spectral-pipeline.yaml` 中有效。請勿在 `.spectral.yaml` 中新增 `functionsDir` 或 `functions` — 該設定必須保持與 Super-Linter 及其他外部工具的相容性。
:::

## CI 管線

GitHub Actions 工作流程（`validate-and-release.yml`）在以下情況執行：

- **排程**：每日 UTC 上午 6 點
- **推送至 main**：當 `scripts/`、`config/` 或 `pyproject.toml` 有變更時
- **手動觸發**：可選擇強制發佈或乾跑模式

此工作流程包含四個工作：

1. **validate** — 下載規格、執行驗證、Spectral lint、修正、Spectral 品質門檻
2. **check-release-needed** — 比較內容雜湊值以決定是否需要發佈
3. **release** — 打包並建立 GitHub Release（僅在內容變更時）
4. **update-docs** — 重新產生 `01-validation-report.mdx`，若有變更則開啟 PR

若任何工作失敗，**notify** 工作會建立一個 GitHub issue。

## 分支與 PR 工作流程

所有變更遵循：**Issue -> 分支 -> PR -> CI 通過 -> 合併**。

- 分支命名：`feature/<issue>-desc`、`fix/<issue>-desc`、`docs/<issue>-desc`
- PR 必須連結 issue（在描述中使用 `Closes #N`）
- 必要的 CI 檢查：`Check linked issues` 和 `Lint Code Base`
- 建議使用壓縮合併；分支在合併後自動刪除

完整工作流程規則請參閱 `CONTRIBUTING.md`。

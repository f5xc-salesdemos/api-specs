---
title: 開發指南
description: 如何設定、執行、測試及貢獻 F5 XC API Specs 專案
i18n:
...
# 安裝 Spectral CLI（linting 階段必需）
npm install -g @stoplight/spectral-cli
```

`make dev-install` 會在 `.venv/` 中建立虛擬環境，並安裝含開發附加套件（pytest、ruff、mypy）的專案。
...
若未設定這些變數，`make validate` 將退回至乾跑模式。
...
```bash
make download        # 擷取上游規格
make validate        # 執行即時 API 測試（需要 API 權杖）
make validate-dry    # 乾跑（不進行 API 呼叫）
...
reports/                # 輸出：驗證與 Spectral 報告
```

## 新增修正方法

若要在調和器中新增 Spectral 自動修正：

1. **新增規則**至 `.spectral.yaml`，設定所需的嚴重性等級。

2. **啟用自動修正**，在 `config/validation.yaml` 的 `spectral.auto_fix` 下設定：
   ```yaml
   spectral:
     auto_fix:
       your-new-rule: true
...
4. **在 `_apply_spectral_fix` 分派器中註冊**：
   ```python
   spectral_fixers = {
       # ... existing fixers ...
...
若要新增含 JavaScript 函式的自訂 Spectral 規則：

1. 在 `spectral/functions/your-rule.js` 中建立函式。匯出一個預設函式，接收 `(targetVal, options, context)` 並回傳 `{message, path}` 物件的陣列。
...
:::caution
自訂函式僅適用於 `spectral-pipeline.yaml`。請勿在 `.spectral.yaml` 中新增 `functionsDir` 或 `functions`——該配置必須保持與 Super-Linter 及其他外部工具的相容性。
:::
...
1. **validate** —— 下載規格、執行驗證、Spectral lint、調和、Spectral 關卡
2. **check-release-needed** —— 比較內容雜湊值以決定是否需要發行
3. **release** —— 打包並建立 GitHub Release（僅在內容有變更時）
4. **update-docs** —— 重新產生 `01-validation-report.mdx`，若有變更則開啟 PR

若任何工作失敗，**notify** 工作會建立一個 GitHub issue。
...
- 建議使用 squash merge；分支在合併後自動刪除

完整的工作流程規則請參閱 `CONTRIBUTING.md`。

---
title: Spectral 检查规则
description: 采用双配置 Spectral 架构，包含基础 OAS 规则和自定义 f5-path-params 函数
i18n:
...
供 GitHub Super-Linter 和其他外部 CI 工具使用。继承 `spectral:oas`（内置 OAS 规则集）。不包含自定义函数——这确保其与无法访问 `spectral/functions/` 目录的工具兼容。
...
Spectral 内置的 `path-params` 规则会错误地标记使用带点参数名（如 `{metadata.namespace}` 和 `{metadata.name}`）的 F5 XC API。内置规则无法解析参数名中的点号，因此会将其报告为未声明的参数。

自定义 `f5-path-params` 函数（`spectral/functions/f5-path-params.js`）通过将完整的 `{placeholder}` 内容与已声明的参数名进行匹配，正确处理了带点号的名称。它同时检查路径级别和操作级别的参数，并报告两个方向的错误：
...
当检测到违规时，协调器会自动修复这些规则。详见[已应用的修复](/03-fixes-applied/)了解每项修复的详细信息。
...
| `f5-path-params` | error | -- | 路径参数必须已声明且被使用 |

:::note
`f5-path-params` 是 error 严重程度的规则，但不会自动修复。违规表示规范存在真正的缺陷，需要人工审查。
:::

### 不可修复的规则

这些规则被降级为 `info` 严重程度，因为无法进行有意义的自动修复：

| 规则 | 严重程度 | 描述 |
|------|----------|-------------|
| `operation-description` | info | 操作应包含描述 |
| `info-description` | info | API 信息应包含描述 |
...
扫描 `specs/original/` 并输出 `reports/spectral_report.json`。协调器将此报告与验证报告一起使用。
...
扫描 `release/specs/` 并输出 `reports/spectral_gate_report.json`。如果违规数超过配置的阈值，则返回退出码 1。

...
`make dev-install` 会在 `.venv/` 中创建虚拟环境，并安装包含开发扩展（pytest、ruff、mypy）的项目。
...
如果未设置这些变量，`make validate` 将回退到干运行模式。
...
make validate-dry    # 干运行（不进行 API 调用）
make spectral-lint   # 在原始规范上运行 Spectral 发现模式
make reconcile       # 应用两份报告中的修复
make spectral-gate   # 在修复后的规范上运行 Spectral 质量门禁
make release         # 构建发布包
```
...
make ci-test        # 以上三项全部运行（用于 CI）
```

如果已安装，pre-commit 钩子会在提交时自动运行：
...
1. 在 `spectral/functions/your-rule.js` 中创建函数。导出一个接收 `(targetVal, options, context)` 的默认函数，并返回一个包含 `{message, path}` 对象的数组。
...
:::caution
自定义函数仅在 `spectral-pipeline.yaml` 中有效。请勿将 `functionsDir` 或 `functions` 添加到 `.spectral.yaml` 中——该配置必须保持与 Super-Linter 及其他外部工具的兼容性。
:::
